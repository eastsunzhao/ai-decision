...
def chat(request: ChatRequest) -> ChatResponse:
    logger.info("chat_request_received scenario_id=%s query_len=%s", request.scenario_id, len(request.query))
    if not scenario_manager.has_scenario(request.scenario_id):
        logger.warning("chat_scenario_not_found scenario_id=%s", request.scenario_id)
    ...
    if not final_state.answer:
        logger.error("chat_answer_empty scenario_id=%s errors=%s", request.scenario_id, final_state.errors)
        raise HTTPException(status_code=500, detail="Failed to generate answer")

    logger.info(
        "chat_request_completed scenario_id=%s citations=%s confidence=%.2f reflection_iterations=%s",
        request.scenario_id,
    ...
    session = session_store.save_session(request.scenario_id, request.query, response.model_dump())
...
def chat_stream(request: ChatRequest) -> StreamingResponse:
    logger.info("chat_stream_request_received scenario_id=%s query_len=%s", request.scenario_id, len(request.query))
    if not scenario_manager.has_scenario(request.scenario_id):
        logger.warning("chat_stream_scenario_not_found scenario_id=%s", request.scenario_id)
        raise HTTPException(status_code=404, detail=f"Scenario {request.scenario_id} not found")

    def event_stream():
        state = ResearchState(scenario_id=request.scenario_id, query=request.query)
        for event in workflow.run_with_events(state):
            if event.get("type") == "final":
                ...
                logger.info(
                    "chat_stream_final scenario_id=%s citations=%s confidence=%.2f",
                    request.scenario_id,
                ...
                logger.info("chat_stream_step scenario_id=%s node=%s status=%s", request.scenario_id, event.get("name"), event.get("status"))
                yield _sse(event)
