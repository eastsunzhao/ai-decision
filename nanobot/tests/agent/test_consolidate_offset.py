...
class TestNewCommandArchival:
    ...
    @pytest.mark.asyncio
<<<<<<< HEAD:tests/test_consolidate_offset.py
    async def test_new_command_does_not_trigger_additional_consolidation(
        self, tmp_path: Path
    ) -> None:
        """/new clears immediately and does not start its own archival/consolidation."""
        from nanobot.agent.loop import AgentLoop
        ...
        assert consolidation_calls == 1, (
            f"Expected only background consolidation, got {consolidation_calls}"
        )
        assert max_active == 1, (
    ...
    @pytest.mark.asyncio
    async def test_new_does_not_wait_for_inflight_consolidation(
        self, tmp_path: Path
    ) -> None:
        """/new clears immediately instead of waiting for in-flight consolidation."""
        from nanobot.agent.loop import AgentLoop
        from nanobot.bus.events import InboundMessage
        from nanobot.bus.queue import MessageBus
        from nanobot.providers.base import LLMResponse
        ...
        assert pending_new.done(), "/new should return immediately while consolidation is in-flight"
        ...
        assert session_after.messages == [], "Session should be cleared immediately on /new"

    @pytest.mark.asyncio
    async def test_new_clears_session_without_archival(self, tmp_path: Path) -> None:
        """/new should clear the session without invoking archive_all consolidation."""
        from nanobot.agent.loop import AgentLoop
        from nanobot.bus.events import InboundMessage
        from nanobot.bus.queue import MessageBus
        from nanobot.providers.base import LLMResponse
...
=======
    async def test_new_clears_session_immediately_even_if_archive_fails(self, tmp_path: Path) -> None:
        ...
        from nanobot.bus.events import InboundMessage
>>>>>>> 33abe915e767f64e43b4392a4658815862d2e5f4:tests/agent/test_consolidate_offset.py
...
>>>>>>> 33abe915e767f64e43b4392a4658815862d2e5f4:tests/agent/test_consolidate_offset.py
        ...
        assert "new session started" in response.content.lower()
<<<<<<< HEAD:tests/test_consolidate_offset.py
        assert consolidation_calls == 0, "/new should not call _consolidate_memory"
        session_after = loop.sessions.get_or_create("cli:test")
        assert len(session_after.messages) == 0, "Session should still be cleared on /new"
