...
def test_pick_consolidation_boundary_fallback_without_next_user(tmp_path, monkeypatch) -> None:
    loop = _make_loop(tmp_path, estimated_tokens=0, context_window_tokens=200)
    session = loop.sessions.get_or_create("cli:test")
    session.messages = [
        {"role": "user", "content": "u1", "timestamp": "2026-01-01T00:00:00"},
        {"role": "assistant", "content": "a1", "timestamp": "2026-01-01T00:00:01"},
        {"role": "tool", "content": "t1", "tool_call_id": "call_1", "name": "read_file", "timestamp": "2026-01-01T00:00:02"},
    ]
    monkeypatch.setattr(memory_module, "estimate_message_tokens", lambda _m: 200)
    boundary = loop.memory_consolidator.pick_consolidation_boundary(session, tokens_to_remove=300)
    assert boundary is not None
    assert boundary[0] == 3


def test_enforce_prompt_budget_trims_old_assistant_tool_messages(tmp_path, monkeypatch) -> None:
    loop = _make_loop(tmp_path, estimated_tokens=0, context_window_tokens=200)
    loop.max_tokens = 10
    loop._PROMPT_TRIM_SAFETY_BUFFER = 0
    token_estimates = iter([(500, "test"), (180, "test")])
    monkeypatch.setattr(
        "nanobot.agent.loop.estimate_prompt_tokens_chain",
        lambda *_args, **_kwargs: next(token_estimates),
    )
    messages = [
        {"role": "system", "content": "sys"},
        {"role": "assistant", "content": "a" * 6000},
        {"role": "tool", "tool_call_id": "call_1", "name": "read_file", "content": "b" * 6000},
        {"role": "user", "content": "latest user"},
    ]
    out = loop._enforce_prompt_budget(messages, tool_defs=[])
    assert len(out[1]["content"]) == loop._PROMPT_TRIM_BLOCK_MAX_CHARS
    assert "trimmed for context budget" not in out[1]["content"]


def test_sanitize_user_visible_text_removes_internal_marker(tmp_path) -> None:
    loop = _make_loop(tmp_path, estimated_tokens=0, context_window_tokens=200)
    cleaned = loop._sanitize_user_visible_text("[trimmed for context budget]")
    assert cleaned == ""
