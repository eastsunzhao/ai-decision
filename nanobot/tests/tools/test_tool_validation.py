...
def test_exec_uses_nb_python_parent_without_resolve(monkeypatch) -> None:
    """NB_PYTHON path pinning should not dereference symlinks via Path.resolve."""
    ...
    assert "Exit code: 0" in result
    assert captured_env["PATH"].split(os.pathsep)[0] == expected_dir


def test_decode_output_bytes_prefers_utf8_over_locale_gbk(monkeypatch) -> None:
    monkeypatch.setattr("nanobot.agent.tools.shell.locale.getpreferredencoding", lambda _: "gbk")
    raw = "当前市场状态".encode("utf-8")
    assert ExecTool._decode_output_bytes(raw) == "当前市场状态"


def test_decode_output_bytes_falls_back_to_gbk_when_utf8_fails(monkeypatch) -> None:
    monkeypatch.setattr("nanobot.agent.tools.shell.locale.getpreferredencoding", lambda _: "utf-8")
    raw = "当前市场状态".encode("gbk")
    assert ExecTool._decode_output_bytes(raw) == "当前市场状态"
