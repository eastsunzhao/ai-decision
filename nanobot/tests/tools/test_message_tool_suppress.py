...
class TestMessageToolSuppressLogic:
    ...
    @pytest.mark.asyncio
    async def test_web_stream_gets_completion_marker_when_message_tool_suppresses(self, tmp_path: Path) -> None:
        loop = _make_loop(tmp_path)
        tool_call = ToolCallRequest(
            id="call1",
            name="message",
            arguments={"content": "Visible update"},
        )
        calls = iter([
            LLMResponse(content="", tool_calls=[tool_call]),
            LLMResponse(content="Done", tool_calls=[]),
        ...
        assert any(item.metadata.get("_message_tool_delivery") for item in outbound)
        assert any(item.metadata.get("_suppressed_final") for item in outbound)
...
class TestMessageToolTurnTracking:
    ...
    def test_sent_in_turn_tracks_same_target(self, tmp_path: Path) -> None:
    ...
    def test_start_turn_resets(self, tmp_path: Path) -> None:
        tool = MessageTool(workspace=tmp_path)
        tool._sent_in_turn = True
        tool.start_turn()
        assert not tool._sent_in_turn
