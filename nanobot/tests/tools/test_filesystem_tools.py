...
class TestFindMatch:
    ...
    def test_fuzzy_line_window_fallback(self):
        content = (
            "## Wave Execution Log\n"
            "\n"
            "### Wave 2\n"
        ...
        assert match is not None
        assert count == 1
        assert "- blocker_reason: (none)" in match
...
class TestEditFileTool:
    ...
    @pytest.mark.asyncio
    async def test_fuzzy_fallback_with_inserted_line(self, tool, tmp_path):
        f = tmp_path / "plan.md"
        f.write_text(
            "## Wave Execution Log\n\n### Wave 1\n- ready_steps: S1, S2\n- blocker_reason: (none)\n",
            encoding="utf-8",
        ...
        result = await tool.execute(
            path=str(f),
            old_text="## Wave Execution Log\n\n### Wave 1\n- ready_steps: S1, S2\n- blocker_reason:\n",
            new_text="## Wave Execution Log\n\n### Wave 1\n- ready_steps: S1, S2\n- blocker_reason: (none)\n",
        )
        assert "Successfully" in result
        assert "- blocker_reason: (none)" in f.read_text(encoding="utf-8")
