...
_CURRENT_LLM_SESSION_KEY: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "nanobot_current_llm_session_key",
    default=None,
)


def get_current_llm_session_key() -> str | None:
    return _CURRENT_LLM_SESSION_KEY.get()


@contextmanager
def llm_session_context(session_key: str | None):
    token = _CURRENT_LLM_SESSION_KEY.set(session_key)
    try:
        yield
    finally:
        _CURRENT_LLM_SESSION_KEY.reset(token)
