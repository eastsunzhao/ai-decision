...
_DEEPSEEK_TRACE_DIR = Path(__file__).resolve().parents[3] / "nanobot_deepseek_trace" / "tracelog"
...
class OpenAICompatProvider(LLMProvider):
    ...
    # ------------------------------------------------------------------
    # DeepSeek trace logging
    # ------------------------------------------------------------------

    def _deepseek_trace_enabled(self) -> bool:
        if not self._spec or self._spec.name != "deepseek":
            return False
        raw = os.environ.get("NANOBOT_DEEPSEEK_TRACE", "")
    ...
    def _trace_jsonable(value: Any) -> Any:
        ...
        if isinstance(value, dict):
            return {
                str(k): OpenAICompatProvider._trace_jsonable(v)
                for k, v in value.items()
            }
        if isinstance(value, (list, tuple)):
            return [OpenAICompatProvider._trace_jsonable(v) for v in value]
        if is_dataclass(value):
            return OpenAICompatProvider._trace_jsonable(asdict(value))
        model_dump = getattr(value, "model_dump", None)
        if callable(model_dump):
            try:
                return OpenAICompatProvider._trace_jsonable(model_dump(mode="json"))
            except TypeError:
                try:
                    return OpenAICompatProvider._trace_jsonable(model_dump())
                except Exception:
                    pass
            except Exception:
                pass
    ...
    @classmethod
    def _trace_response(cls, response: LLMResponse) -> dict[str, Any]:
        return {
            "content": response.content,
            "reasoning_content": response.reasoning_content,
            "tool_calls": [cls._trace_jsonable(tc) for tc in response.tool_calls],
            "finish_reason": response.finish_reason,
            "usage": cls._trace_jsonable(response.usage),
        }

    @classmethod
    def _trace_error(cls, response: LLMResponse) -> dict[str, Any]:
        return {
            **cls._trace_response(response),
    ...
    @staticmethod
    def _trace_session_fragment() -> str:
        session_key = get_current_llm_session_key()
        if not session_key:
            return "none"
        tail = session_key.split(":")[-1]
        safe = "".join(ch for ch in tail if ch.isalnum()).lower()
        return (safe[:4] or "none")

    @classmethod
    def _default_deepseek_trace_path(cls) -> Path:
        stamp = datetime.now().strftime("%y%m%d")
        return _DEEPSEEK_TRACE_DIR / f"deepseek_trace_{stamp}_{cls._trace_session_fragment()}.jsonl"
    ...
    def _write_deepseek_trace(
        ...
        *,
        ...
        error: LLMResponse | None = None,
    ) -> None:
        if not self._deepseek_trace_enabled():
            return
        try:
            raw_path = os.environ.get("NANOBOT_DEEPSEEK_TRACE_PATH")
            path = Path(raw_path).expanduser() if raw_path else self._default_deepseek_trace_path()
            ...
            record: dict[str, Any] = {
                ...
                "payload": self._trace_jsonable(payload),
            }
            if response is not None:
                record["response"] = self._trace_response(response)
            if error is not None:
                record["error"] = self._trace_error(error)
            with open(path, "a", encoding="utf-8") as f:
                f.write(json.dumps(record, ensure_ascii=False) + "\n")
        except Exception as e:
            logger.warning("Failed to write DeepSeek trace log: {}", e)
    ...
    async def chat(
        ...
        try:
            ...
            trace_payload = dict(kwargs)
            result = self._parse(await self._client.chat.completions.create(**kwargs))
            self._write_deepseek_trace(mode="chat", payload=trace_payload, response=result)
            return result
        except Exception as e:
            result = self._handle_error(e, spec=self._spec, api_base=self.api_base)
            if trace_payload is not None:
                self._write_deepseek_trace(mode="chat", payload=trace_payload, error=result)
            return result
    ...
    async def chat_stream(
        ...
        try:
            ...
            self._write_deepseek_trace(mode="stream", payload=trace_payload, response=result)
            return result
            ...
            if trace_payload is not None:
                self._write_deepseek_trace(mode="stream", payload=trace_payload, error=result)
            return result
        except Exception as e:
            result = self._handle_error(e, spec=self._spec, api_base=self.api_base)
            if trace_payload is not None:
