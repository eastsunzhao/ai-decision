...
def on_progress_accepts_trace_meta(cb: Callable[..., Any]) -> bool:
    try:
        sig = inspect.signature(cb)
    except (TypeError, ValueError):
        return False
    if any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
        return True
    return "trace_meta" in sig.parameters
...
async def invoke_on_progress(
    ...
    *,
    ...
    if trace_meta and on_progress_accepts_trace_meta(on_progress):
        kwargs["trace_meta"] = trace_meta
    if len(kwargs) > 1:
