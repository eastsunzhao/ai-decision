...
class AgentDefaults(Base):
    ...
    iteration_budget: Literal["low", "medium", "high", "extra_high"] = "medium"
    ...
    system_prompt_files: list[str] = Field(
        default_factory=lambda: ["TOOLS.md"],
    )  # Workspace Markdown files to include in the system prompt; [] disables workspace bootstrap files
    ...
    def memory_window(self) -> int:
        """Backward-compatible alias for removed legacy setting."""
        return 100
...
class WebSearchConfig(Base):
    ...
    provider: str = "jina"  # jina, brave, tavily, duckduckgo, searxng, kagi, olostep
    api_key: str = ""
...
class ToolsConfig(Base):
    ...
    sync_workspace_skills: bool = False  # Web runtime syncs repo skills into each user's _agent workspace.
